...
// API Configuration
export const domainUrl = "103.105.110.250:81";
export const domain = `http://${domainUrl}`;
const url = `${domain}/l2s/api`;
export const ispName = 'Spacecom Local';
...
class ApiService {
  ...
  async adminDetails() {
    ...
    try {
      const res = await fetch(`http://${domainUrl}/tmp/isp_details.json`, options);
  ...
  private formatDate(dateString: string, format: string): string {
    try {
      ...
      if (format === 'DD-MMM,YY HH:mm' && dateString.match(/^\d{1,2}-\d{2}-\d{4}\s+\d{1,2}:\d{2}$/)) {
        const parts = dateString.split(' ');
